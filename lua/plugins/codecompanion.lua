return {
    ...
    require("codecompanion").setup({
      ...
      strategies = {
        chat = {
          ...
          tools = {
            opts = {
              -- Read-only tools are attached to every chat by default so the model can
              -- browse the project on its own; nothing here can write or execute anything.
              default_tools = { "read_file", "grep_search", "file_search", "list_code_usages", "get_changed_files" },
            },
            ["read_file"] = {
            ...
            ["insert_edit_into_file"] = {
              opts = {
                require_approval_before = { buffer = true, file = true }, -- Ask before editing, not just after.
              },
            },
            ["create_file"] = {
  ...
  keys = {
    {
      '<leader>ac',
      '<cmd>CodeCompanionChat #{buffer} #{lsp} This is only to initialize the model, only respond with "Ready!" when ready.<cr>',
      mode = { "n", "v" },
